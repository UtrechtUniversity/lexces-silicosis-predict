...
# Misclassification parameters were obtained from data reported in a 
# recent study in artificial stone benchtop industry workers
# https://onlinelibrary.wiley.com/doi/10.1111/resp.14755. These parameters
# are found in the Silicosis_diagnostic_rule.qmd document. 

### This is disabled by default. Use only when running script independently
### from the main qmd file, by deleting the following "#" character:

# source("R/scripts/Hoy_CXR_HRCT_dx_performance.R")
...
for (i in 1:num_repetitions) {
  ...
  # Sample Sensitivity (Sn) from a Beta distribution
  Sn_sim <- rbeta(1, TP, FN)
  
  # Compute Specificity (Sp) using the DOR formula
  Sp_sim <- calculate_Sp(Sn_sim, DOR)
...
write.csv(
  ROC_results, 
  file = paste0(tabfolder, "/Hoy_non-dif_low-med-high.csv"),
  row.names = F
)


cat("Simulation complete! Results saved in the output_tables folder as Hoy_non-dif_low-med-high.csv\n")
